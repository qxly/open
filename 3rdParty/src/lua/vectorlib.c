...
double *newvector(lua_State *L)
{
	double *v=(double *)lua_newuserdata(L, sizeof(double)*3);
...
int vector_mul(lua_State *L)
{
	...
	if(v)
	{
		if(vl_isvector(L,2))
		{
			double *v2=(double *)lua_touserdata(L,2);
			double res=v[0]*v2[0] + v[1]*v2[1] + v[2]*v2[2];
