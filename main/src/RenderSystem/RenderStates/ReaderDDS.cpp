...
namespace open
{
		...
		int32 packing = 1, int32 slice_packing = 1, int32 image_packing = 1)
	{
		...
		return Texture::computeImageSizeInBytes(width, height, depth, (PixelFormat)pixelFormat, pixelType, packing, slice_packing, image_packing);
	}
	...
	ITexture* ReadDDSFile(char* ddsBuffer, int32 bufferSize)
	{
		...
		ITexture* texture = RenderSystemFactory::s_factory->createTexture();
		...
		uint32* mipmap_offsets = nullptr;
		if (ddsd.dwMipMapCount > 1)
		{
			unsigned numMipmaps = Texture::computeNumberOfMipmapLevels(s, t, r);
			...
			for (unsigned int k = 0; k < numMipmaps - 1; ++k)
			{
				...
				sizeWithMipmaps += Texture::computeImageSizeInBytes(width, height, depth, (PixelFormat)pixelFormat, dataType, packing);
			}
		}
